...
const login = async (req, res, next) => {
	...
	if (isFromBeneficiary) {
		isValidPassword = existingUser.health_card_id === password;
		console.log('here', existingUser);
		console.log('here', existingUser.health_card_id, password);
	} else {
		try {
		...
		token = jwt.sign(
			{
				...
				profil_type: isFromBeneficiary ? (existingUser?.profil ? existingUser?.profil : 'BENEFICIARY') : existingUser?.profil_type,
			},
			'supersecret_dont_share', //process.env.JWT_KEY, // private key
			{ expiresIn: '24h' }
		);
	} catch (err) {
		return res.json({
