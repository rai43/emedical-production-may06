...
const createBulkMedications = async (req, res, next) => {
	const errors = validationResult(req);
	...
	try {
		createdMedications = await Medication.insertMany(
			medications.map((elt) => {
				return {
					commercial_name: elt[0],
					price: elt[1],
					dci: elt[2],
					therapeutic_class: elt[3],
					presentation: elt[4],
				};
			})
		);
		...
		console.log(err);
		return next(new HttpError('Impossible de créer les médicaments', 500));
	}

	res.status(201).json({ medications: createdMedications });
};
...
exports.createBulkMedications = createBulkMedications;
